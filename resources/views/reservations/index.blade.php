...
@section('content')
                                            ...
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    ...
                                                    <select id="paymentMethod" data-placeholder="@lang('front.select')"
                                                        ...
                                                        style="width:100%;">
                                                        <option
                                                            {{ $settings['default_payment'] == 'cash' ? 'selected' : '' }}
                                                            value="cash">
                                                            @lang('front.cash')
                                                        </option>
                                                        <option
                                                            {{ $settings['default_payment'] == 'delayed' ? 'selected' : '' }}
                                                            value="delayed">
                                            ...
                                            <div class="box-body">
                                                <div class="nav-tabs-">
                                                    <ul class="nav nav-tabs">
                                                        ...
                                                        @endforeach
                                                        <li class="pull-right header"><i class="fa fa-th"></i>
    ...
    <div rel="flipInX" id="fullCalModal" class="modal fade">
...
@push('css')
    ...
    <link rel="stylesheet" href="{{ asset('front') }}/plugins/fullcalendar/fullcalendar.print.css" media="print">
    <link rel="stylesheet" href="{{ asset('front/plugins') }}/datetimepicker/css/bootstrap-datetimepicker.min.css">
    <style>
        .btn-app {
            margin: 0 5px 10px 0;
        }
...
@push('js')
    <!-- fullCalendar 2.2.5 -->
    <script src="{{ asset('front') }}/dist/js/moment.min.js"></script>
    <script src="{{ asset('front') }}/plugins/fullcalendar/fullcalendar.min.js"></script>
    <script src="{{ asset('front') }}/plugins/datetimepicker/js/bootstrap-datetimepicker.min.js"></script>
    ...
    <script>
        $(function() {
            ...
            /* initialize the external events
             -----------------------------------------------------------------*/
            function ini_events(ele, url, id, user) {
                if (id === undefined) {
                    id = '';
                }
                if (url === undefined) {
            ...
            $('#calendar').fullCalendar({
                ...
                drop: function(date, allDay) { // this function is called when something is dropped
                    ...
                    updateEventTimes(copiedEventObject, isNew);

                },
                eventDrop: function(event, delta, revertFunc, jsEvent, ui, view) {
                    //console.log(event.id);
                    updateEventTimes(event, false);
                },
                ...
                eventRender: function(event, element) {
                    ...
                    @if (auth()->user()->can('Delete Events'))
                        if (url[3] != "invoice") {
                            element.append('<i  onclick="event.preventDefault();removeEvent(' + event
                                ._id + ',\'' + url[3] +
                                '\')" class="fa fa-trash removeEvent"  aria-hidden="true"></i>');
                        }
                    @endif
                },
            ...
            $("#add-new-event").click(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        ini_events(event, data.eventLink, data.id, data.user);
        ...
        function updateEventTimes(event, isNew) {
            //console.log(event);
            if (isNew === undefined) {
                isNew = false;
            }
        ...
        function removeEvent(eventId, type) {
            //alert(url);
            isRemoveFlag = true;
            swal({
                title: "تأكيد الحذف",
                text: "هل متأكد من حذف هذا الحدث؟",
                type: "warning",
